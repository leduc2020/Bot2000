...
module.exports.handleReply = async function({ handleReply: $, api, Currencies, event, Users }) {};
  ...
  if (/^vy\b/i.test(body)) {
    const prompt = body.replace(/^vy\b/i, "").trim();
    if (!prompt) return;
    ...
    try {
      ...
      const reply = result?.response?.text?.() || "Vy chưa nghĩ ra gì để nói luôn á 😢";

      return api.sendMessage(reply, threadID, async () => {
        try {
          const lower = reply.toLowerCase();
          let emo = "🤖";
          if (lower.includes("vui") || lower.includes("haha") || lower.includes("hehe") || lower.includes("sướng") || lower.includes("thích")) emo = "😄";
          else if (lower.includes("buồn") || lower.includes("khóc") || lower.includes("tệ") || lower.includes("chán") || lower.includes("không sao")) emo = "😢";
          else if (lower.includes("giận") || lower.includes("bực") || lower.includes("nổi điên") || lower.includes("cay")) emo = "😡";
          else if (lower.includes("cảm ơn") || lower.includes("thank") || lower.includes("biết ơn") || lower.includes("yêu")) emo = "❤️";
          else if (lower.includes("đúng") || lower.includes("chính xác") || lower.includes("hợp lý") || lower.includes("hợp lí")) emo = "✅";
          else if (lower.includes("sai") || lower.includes("không đúng") || lower.includes("sai rồi") || lower.includes("nhầm")) emo = "❌";
          else if (lower.includes("?") || lower.includes("hỏi") || lower.includes("ai") || lower.includes("gì") || lower.includes("sao") || lower.includes("nào")) emo = "❓";
          else if (lower.includes("ok") || lower.includes("oke") || lower.includes("được") || lower.includes("ổn")) emo = "👌";
          else if (lower.includes("ngủ") || lower.includes("ngon giấc") || lower.includes("mơ đẹp")) emo = "😴";
          else if (lower.includes("đẹp") || lower.includes("xinh") || lower.includes("tuyệt vời")) emo = "😍";
          else if (lower.includes("ngạc nhiên") || lower.includes("thật sao") || lower.includes("wow")) emo = "😲";
          else if (lower.includes("hài") || lower.includes("cười") || lower.includes("vcl")) emo = "😂";
          if (lower.includes("vui") || lower.includes("tuyệt") || lower.includes("haha")) emo = "😄";
          else if (lower.includes("buồn") || lower.includes("xin lỗi") || lower.includes("không được")) emo = "😢";
          else if (lower.includes("giận") || lower.includes("bực") || lower.includes("tức")) emo = "😡";
          else if (lower.includes("cảm ơn") || lower.includes("thank")) emo = "❤️";
          await api.setMessageReaction(emo, messageID, threadID, true);
        } catch (e) {}
      }, messageID);

    } catch (err) {
      console.error("Lỗi AI phản hồi:", err.message);
      return api.sendMessage("Vy bị lag nhẹ á, thử lại sau nhen 😢", threadID, messageID);
    }
  }
